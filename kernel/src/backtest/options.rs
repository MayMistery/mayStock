...
use super::{
    empty_result, BacktestConfig, BacktestResult, EquityPoint, ExitReason, Metrics, Trade,
};
...
pub fn run(
    ...
    let costs = super::resolve_costs(strategy, config)?;
    ...
    // The day the loss breaker resets on is the market's, not UTC's.
    ...
    for index in 0..candles.len() {
            ...
            if let Some(open) = position.as_ref() {
                if Some(open.kind.direction()) != target {
                    ...
                    let fee = options::option_fee(
                        &costs.fees, OrderSide::Sell, candle.open, exit, open.units,
                        spec.fee_cap_pct_of_premium);
                    close_position(
                ...
                if let Some(direction) = target {
                    ...
                    if can_enter(since_exit, manifest.risk.cooldown_bars) {
                        open_position(
                            ...
                            candle.ts_ms, index, sigma_prev, slippage, &costs.fees,
                            &limits, &mut cash,
                        );
                    }
                }
        ...
        if let Some(open) = position.as_ref() {
            ...
            if let Some(reason) = hit {
                ...
                let fee = options::option_fee(
                    &costs.fees, OrderSide::Sell, candle.close, exit, open.units,
                    spec.fee_cap_pct_of_premium);
                close_position(
    ...
    if let Some(open) = position.as_ref() {
        ...
        let fee = options::option_fee(
            &costs.fees, OrderSide::Sell, last.close, exit, open.units,
            spec.fee_cap_pct_of_premium);
        close_position(
...
fn open_position(
    ...
    let fee = options::option_fee(fees, OrderSide::Buy, spot, paid, units, spec.fee_cap_pct_of_premium);
