...
pub fn option_fee(
    ...
    let on_notional = fee_bps / 10_000.0 * spot * units;
...
mod tests {
    ...
    fn the_fee_is_on_notional_but_capped_by_the_premium() {
        ...
        assert!((option_fee(3.0, 80_000.0, 500.0, 1.0, 12.5) - 24.0).abs() < 1e-9);
        // A 50 premium caps the fee at 6.25.
        assert!((option_fee(3.0, 80_000.0, 50.0, 1.0, 12.5) - 6.25).abs() < 1e-9);
    }
