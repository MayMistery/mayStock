...
extension AppState {
    ...
    /// Derivative positions the exchange holds on instruments no strategy's
    /// book has a position in: opened by hand, by another program, or before
    /// this install existed. Shown as holdings — they are the account's price
    /// risk whoever opened them — and counted in exposure by the runner.
    var externalPositions: [ExchangePosition] {
        let held = Set(ledger.positions.values.filter { !$0.isFlat }.map(\.instId))
