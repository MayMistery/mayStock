...
struct RobustnessBadge: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            Image(systemName: icon).font(.system(size: compact ? 9 : 11))
...
struct BacktestWindowCard: View {
    ...
    private func miniRow(_ label: String, _ value: String, tint: Color? = nil) -> some View {
        HStack(spacing: 4) {
            ...
            Spacer(minLength: 2)
            Text(value)
                .font(Theme.Text.captionMedium).monospacedDigit()
...
struct BacktestMetricGrid: View {
    ...
    private func cell(_ label: String, _ value: String, tint: Color? = nil) -> some View {
        VStack(alignment: .leading, spacing: 1) {
            Text(label).font(Theme.Text.caption).foregroundStyle(.secondary)
            Text(value)
                .font(Theme.Text.number).monospacedDigit()
