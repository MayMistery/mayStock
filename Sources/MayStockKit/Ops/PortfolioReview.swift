...
public struct ReviewSnapshot: Sendable {
    ...
    /// The exchange's own totals, keyed by instrument. Nil when the review
    /// could not reach the venue — which must read as "not checked", never as
    /// "checked and fine".
    public var exchangeTotals: [String: ExchangeBookTotals]?
    ...
    public init(
        ...
        exchangeTotals: [String: ExchangeBookTotals]? = nil
    ) {
...
public enum PortfolioReview {
    public static func run(_ snapshot: ReviewSnapshot, policy: ReviewPolicy) -> ReviewResult {
        ...
        findings += bookDrift(snapshot, policy)
    ...
    /// Does the book's money agree with the account's money?
    ///
    /// Every other check here reads our own numbers and asks whether they look
    /// reasonable. That cannot catch an arithmetic error, because a wrong
    /// figure stays perfectly consistent with every figure derived from it: a
    /// P&L booked at ten times its size looked exactly as plausible as the
    /// truth, and the strategy curve, the exposure and the drawdown all agreed
    /// with it. Only the exchange is outside that loop.
    ///
    /// The three fields compared are the only places money enters the book, so
    /// between them they cover the whole class — a fabricated multiplier, a
    /// double-booked settlement, a fill ingested twice or missed entirely.
    static func bookDrift(_ s: ReviewSnapshot, _ policy: ReviewPolicy) -> [ReviewFinding] {
        guard let totals = s.exchangeTotals else {
            // Not reachable is a state, not a pass. Saying so costs one info
            // line; leaving it silent means a permanently broken comparison
            // reads identically to a permanently clean one.
            return [ReviewFinding(
                code: "ledger.uncheckable",
                severity: .info,
                title: "这一轮没能和交易所对账",
                detail: "取不到交易所账单，账本的已实现盈亏／手续费／资金费本轮未经外部核对。",
                remedy: "偶发可忽略；若连续多轮如此，先查 okx CLI 能不能用")]
        }
        ...
        let optionBooks = s.positions.values
            .filter { InstrumentType.of(instId: $0.instId) == .option && !$0.isFlat }
        ...
        for (instId, total) in totals.sorted(by: { $0.key < $1.key }) {
            ...
            // instrument total cannot be split without guessing.
            let owners = s.positions.values.filter { $0.instId == instId }
            guard owners.count == 1, let book = owners.first else { continue }

            // OKX serves a bounded window of bills. A book that predates the
            // window differs for the window's reason, not the book's, and a
            // check that cannot tell those apart trains people to ignore it.
            //
            // Decided on trade ids, not timestamps: a bill is generated by the
            // fill it bills for, so it lands on or milliseconds after it, and
            // comparing stamps declared a complete window incomplete.
            let ours = s.fills.filter { $0.instId == instId }
            let missing = ours.map(\.id).filter { !total.tradeIds.contains($0) }
            if !missing.isEmpty {
                findings.append(ReviewFinding(
                    code: "ledger.uncheckable",
                    severity: .info,
                    title: "账单窗口盖不住持仓历史",
                    detail: "\(instId) 有 \(missing.count)/\(ours.count) 笔成交不在交易所账单窗口内"
                        + "（账单最早回溯到 \(Self.stamp(total.earliestBillAt))），"
                        + "差额无法归因，本轮不比。",
                    remedy: "要长期可核对，就把账单落到本地留存"))
                continue
            }

            let deltas: [(String, Double)] = [
                ("已实现盈亏", book.realisedPnL - total.realisedPnL),
                ("手续费", book.feesPaid - total.fees),
                ("资金费", (book.fundingPaid ?? 0) - total.funding),
            ]
            let off = deltas.filter { abs($0.1) > policy.ledgerDriftToleranceQuote }
            guard !off.isEmpty else { continue }
            findings.append(ReviewFinding(
                code: "ledger.drift",
                severity: .critical,
                title: "账本和交易所对不上",
                detail: "\(book.strategyId) \(instId)："
                    + off.map { String(format: "%@ 账上比账单多 %+.4f", $0.0, $0.1) }
                        .joined(separator: "，")
                    + "。这是记账错误，不是行情 —— 两边数的是同一批成交。",
                remedy: "以交易所账单为准。查这笔差额是哪一笔成交或结算算错的，"
                    + "修完用重放成交的方式把账改回来，不要直接改总数"))
        }
        return findings
    }
