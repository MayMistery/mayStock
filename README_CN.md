...
- **美股同列**：自选里 `TSLA`、`QQQ` 和币对并排。每家交易所各有自己的行情源，
  都收在同一个 `MarketFeed` 端口后面：美股在 `schwabctl` 登录后读嘉信 Trader API 的
  官方行情，没登录时自动退到 Yahoo Finance 图表接口（页脚会写明当前来源），涨跌按昨收算，
  盘前/交易中/盘后/休市在每个界面都标出来，图表按纽约时间标注并把休市时段压缩掉，
  会话装不下的 4H 周期直接不提供；回测用内核的纽交所日历，周末与节假日不算缺口。
- **嘉信交易**：`schwabctl`（Rust，`schwabctl/`，`make.sh install` 装进 App 包并链接到 PATH）
...
- **终端窗口**：一个带侧栏的窗口——总览（账户权益与带空洞的权益曲线、持仓、交易所余额、
  最近成交、引擎的每条提示）、行情（大图表版自选列表，以及每个标的在菜单栏怎么显示）、
  策略、告警、账户与连接、设置。每页顶部固定一条环境栏：当前在哪个账户、连不连得上、急停。
...
- **交易联动**：走 OKX 官方 CLI（Agent Trade Kit）。默认模拟盘（`--demo`）；
  实盘需显式解锁 + 逐策略确认。**模拟盘与实盘是两个账户、两套 API Key**，
  所以每个环境各配一个 CLI profile；切换前先验证目标账户（只读余额），说明会停掉什么，
  并在切换边界重启交易循环——在一个账户上做的决定不会在另一个账户上执行。
  MayStock 不接触、不存储任何 API Key。**回测只用公开行情，不需要任何凭证。**
- **现货、永续、期权**：清单声明自己的市场；`OPTION` 策略读标的的 K 线，
  做多信号买看涨、做空信号买看跌，只买不卖。回测按 Black–Scholes 用实现波动率
  给合约定价（并标明是模型价）；实盘读交易所期权链和盘口、IOC 限价成交，
  挑合约的规则与回测是同一个函数。权利金以结算币（BTC）支付，账户没有、又不能借时，
  下单前就拒绝并写明缺口。
...
macOS 27 SDK 下 Command Line Tools 缺少 SwiftUI 宏插件，构建脚本会自动到已安装的
Xcode 里找；直接 `swift build` 需要自己加 `-Xswiftc -plugin-path`。
...
```bash
./Scripts/make.sh verify   # Release 构建 + 单测 + 连真实 OKX 的 E2E
```
...
```bash
./Scripts/make.sh snapshot   # 把悬浮面板与终端每一页画成 dist/snapshots/*.png
```

菜单栏应用没有窗口可截，所以 App 自己会画：用状态目录的**副本**、不启动交易循环、
在真实的离屏窗口里渲染。改布局先看图再发布。
...
| 悬停菜单栏项 | 浮现图表面板（不抢焦点） |
| 左键点击 | 钉住 / 取消钉住面板 |
| 右键点击 | 打开终端 / 行情 / 策略 / 切换模拟盘·实盘 / 账户与连接 / 关于 / 退出 |
| 面板内 + | 一键添加当前价告警 |
