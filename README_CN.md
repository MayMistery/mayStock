...
- **美股同列**：自选里 `TSLA`、`QQQ` 和币对并排。每家交易所各有自己的行情源，
  都收在同一个 `MarketFeed` 端口后面：美股暂由 Yahoo Finance 图表接口提供（无需 key，
  嘉信 Trader API 审批期间的过渡源），涨跌按昨收算，盘前/交易中/盘后/休市在每个界面
  都标出来，图表按纽约时间标注并把休市时段压缩掉，会话装不下的 4H 周期直接不提供；
  回测用内核的纽交所日历，周末与节假日不算缺口。
